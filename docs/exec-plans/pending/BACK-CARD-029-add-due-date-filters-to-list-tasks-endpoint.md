...
## Acceptance criteria
- [ ] `GET /v1/tasks` without query params returns the same results as before.
- [ ] `GET /v1/tasks?due_from=YYYY-MM-DD` returns only tasks with `due_date >= due_from`.
- [ ] `GET /v1/tasks?due_to=YYYY-MM-DD` returns only tasks with `due_date <= due_to`.
- [ ] `GET /v1/tasks?due_from=...&due_to=...` returns only tasks with `due_date` within the inclusive range.
- [ ] When due filters are used, tasks with `due_date = null` are not returned.
- [ ] Invalid `due_from`/`due_to` format returns 400 with `{ "error": { "code": "...", "message": "..." } }`.
- [ ] `due_from > due_to` returns 400 with the standard error shape.

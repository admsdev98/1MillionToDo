...
3. Add an access helper used by:
   - `GET /v1/tasks/:id`
   - `PATCH /v1/tasks/:id`
   - `DELETE /v1/tasks/:id`
   - share endpoints
4. For shared tasks, block PATCH/DELETE/share with 403 and a clear error code.

## Acceptance criteria
- [ ] Recipient can see shared tasks in list and fetch them by id.
- [ ] Recipient cannot PATCH/DELETE/share a shared task (403).
- [ ] Owner can add/remove shares.
- [ ] Sharing does not allow cross-tenant access unless explicitly shared.

## Evidence to attach when completed
- `curl` walkthrough with two users showing share + recipient read-only behavior.
- Example list response showing a shared indicator (e.g., `isShared: true`).

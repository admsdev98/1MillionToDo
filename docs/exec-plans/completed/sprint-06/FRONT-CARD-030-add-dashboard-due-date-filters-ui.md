...
## Acceptance criteria
- [x] Dashboard shows a due-date filter UI with from/to date inputs and Apply/Clear actions.
- [x] Applying a valid range calls `GET /v1/tasks` with `due_from`/`due_to` and updates the list.
- [x] Clearing filters reloads the full list (no query params).
- [x] If both bounds are set and `from > to`, the UI blocks applying and shows a banner error (no API call).
- [x] Filter labels and banner message render correctly in ES and EN.

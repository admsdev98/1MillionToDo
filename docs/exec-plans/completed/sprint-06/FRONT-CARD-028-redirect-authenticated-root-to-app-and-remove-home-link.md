...
## Acceptance criteria
- [x] With a valid JWT in `localStorage`, visiting `/` lands on `/app` without rendering the landing view.
- [x] Without a JWT, visiting `/` renders the landing view.
- [x] App pages (`/app`, `/app/settings`) have no Home/Inicio link in the header navigation.
- [x] The brand link in app pages navigates to `/app`.
- [x] ES default and EN alternate produce consistent navigation labels and routes.
